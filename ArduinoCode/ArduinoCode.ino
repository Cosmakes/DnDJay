...
void handleSerial()
{
  while (Serial1.available())
  {
    ...
    {
      String list = cmd.substring(7);
      for (int i = 0; i < 4; i++) themes[i] = "";

      int i = 0;
      while (list.length() && i < 4)
      {
    ...
    // --- Load battlemaps ---
    else if (cmd.startsWith("Battlemaps:"))
    {
      String list = cmd.substring(11);
      for (int i = 0; i < 4; i++) battlemaps[i] = "";
      ...
      while (list.length() && i < 4)
      {
        ...
        battlemaps[i++] = list.substring(0, sep);
        list = list.substring(sep + 1);
        list.trim();
      }
      drawCurrentMenu();
    ...
    // --- Select theme ---
    else if (cmd.startsWith("Select theme:"))
    {
...
void loop()
{
  ...
  if (now - lastJoyReport > 100)
  {
    ...
    float pot = analogRead(Potentiometer) * (5.0 / 1023.0);
